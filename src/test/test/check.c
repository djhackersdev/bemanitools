...
static void _Noreturn __attribute__((format(printf, 4, 5)))
fail(const char *file, int line, const char *func, const char *fmt, ...)
{
    va_list ap;
...
void _Noreturn check_bool_neq_failed(
    const char *file,
    int line,
    const char *func,
    char *expr,
    ...
    size_t expected_size)
{
    ...
    hex_encode_uc(
        expected, expected_size, expected_data, sizeof(expected_data));
