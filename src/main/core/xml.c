 ...
 */
static size_t
get_zero_terminated_array_attributes(struct xml_attribute **attributes)
{
    size_t elements = 0;
 ...
 */
static void xml_parser_error(
    struct xml_parser *parser,
    enum xml_parser_offset offset,
    char const *message)
{
    int row = 0;
    int column = 0;
...
#undef max
    ...
    if (NO_CHARACTER != offset) {
        fprintf(
            stderr,
            "xml_parser_error at %i:%i (is %c): %s\n",
            row + 1,
            column,
...
#define min(X, Y) ((X) < (Y) ? (X) : (Y))
    ...
    memcpy(
        ...
        min(n, parser->length - parser->position));
    consumed[n] = 0;
#undef min
 ...
 */
static struct xml_attribute **
xml_find_attributes(struct xml_parser *parser, struct xml_string *tag_open)
{
    ...
    for (token = xml_strtok_r(NULL, " ", &rest); token != NULL;
         token = xml_strtok_r(NULL, " ", &rest)) {
        ...
        attributes = realloc(
            attributes, (new_elements + 1) * sizeof(struct xml_attribute *));
...
static struct xml_string *xml_parse_tag_end(struct xml_parser *parser)
{
    ...
    if ('>' != xml_parser_peek(parser, CURRENT_CHARACTER)) {
        xml_parser_error(
            parser, CURRENT_CHARACTER, "xml_parse_tag_end::expected tag end");
        return 0;
    }
    xml_parser_consume(parser, 1);
...
static struct xml_string *xml_parse_content(struct xml_parser *parser)
{
    ...
    if ('<' != xml_parser_peek(parser, CURRENT_CHARACTER)) {
        xml_parser_error(
            parser, CURRENT_CHARACTER, "xml_parse_content::expected <");
        return 0;
    }
...
static struct xml_node *xml_parse_node(struct xml_parser *parser)
{
    ...
    if ('<' != xml_parser_peek(parser, CURRENT_CHARACTER)) {
        ...
        while ('/' != xml_parser_peek(parser, NEXT_CHARACTER)) {
            ...
            if (!child) {
                xml_parser_error(
                    parser, NEXT_CHARACTER, "xml_parse_node::child");
                goto exit_failure;
            }
            ...
            children = realloc(
                children, (new_elements + 1) * sizeof(struct xml_node *));
...
struct xml_document *xml_parse_document(uint8_t *buffer, size_t length)
{
     ...
     */
    struct xml_parser parser = {
        .buffer = buffer, .position = 0, .length = length};

    /* An empty buffer can never contain a valid document
     */
    if (!length) {
        xml_parser_error(
            &parser, NO_CHARACTER, "xml_parse_document::length equals zero");
        return 0;
    }
...
struct xml_document *xml_open_document(FILE *source)
{
    ...
    while (!feof(source)) {
        ...
        size_t read = fread(
            &buffer[document_length], sizeof(uint8_t), read_chunk, source);
 ...
 */
struct xml_string *
xml_node_attribute_name(struct xml_node *node, size_t attribute)
{
 ...
 */
struct xml_string *
xml_node_attribute_content(struct xml_node *node, size_t attribute)
{
    if (attribute >= xml_node_attributes(node)) {
        return 0;
    }
 ...
 */
struct xml_node *
xml_easy_child(struct xml_node *node, uint8_t const *child_name, ...)
{
    ...
    while (child_name) {
        ...
        struct xml_string cn = {
            .buffer = child_name, .length = strlen(child_name)};
