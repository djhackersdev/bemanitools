...
#define JBHOOK1_CONFIG_EAMUSE_DEFAULT_PCBID_VALUE_LEN sizeof(security_id_default)
#define JBHOOK1_CONFIG_EAMUSE_DEFAULT_EAMID_VALUE security_id_default
#define JBHOOK1_CONFIG_EAMUSE_DEFAULT_EAMID_VALUE_LEN sizeof(security_id_default)
...
void jbhook1_config_eamuse_init(struct cconfig* config)
{
    ...
    cconfig_util_set_data(config, JBHOOK1_CONFIG_EAMUSE_PCBID_KEY, 
        (uint8_t*) &JBHOOK1_CONFIG_EAMUSE_DEFAULT_PCBID_VALUE, 
        JBHOOK1_CONFIG_EAMUSE_DEFAULT_PCBID_VALUE_LEN, "PCBID");

    cconfig_util_set_data(config, JBHOOK1_CONFIG_EAMUSE_EAMID_KEY, 
        (uint8_t*) &JBHOOK1_CONFIG_EAMUSE_DEFAULT_EAMID_VALUE, 
        JBHOOK1_CONFIG_EAMUSE_DEFAULT_EAMID_VALUE_LEN, "EAMID");
}
        ...
        struct cconfig* config)
{
    ...
    if (!cconfig_util_get_str(config, JBHOOK1_CONFIG_EAMUSE_SERVER_KEY,
            server_url, sizeof(server_url),
            JBHOOK1_CONFIG_EAMUSE_DEFAULT_SERVER_VALUE)) {
        log_warning("Invalid value for key '%s' specified, fallback "
            "to default '%s'", JBHOOK1_CONFIG_EAMUSE_SERVER_KEY,
            JBHOOK1_CONFIG_EAMUSE_DEFAULT_SERVER_VALUE);
    }

    if (!net_str_parse(server_url, &config_eamuse->server)) {
        memcpy(&config_eamuse->server, &jbhook1_eamuse_default_server,
    ...
    if (!cconfig_util_get_data(config, JBHOOK1_CONFIG_EAMUSE_PCBID_KEY, 
    ...
    if (!cconfig_util_get_data(config, JBHOOK1_CONFIG_EAMUSE_EAMID_KEY, 
            (uint8_t*) &config_eamuse->eamid,
            JBHOOK1_CONFIG_EAMUSE_DEFAULT_EAMID_VALUE_LEN,
            (uint8_t*) &JBHOOK1_CONFIG_EAMUSE_DEFAULT_EAMID_VALUE)) {
        tmp = security_id_to_str(&JBHOOK1_CONFIG_EAMUSE_DEFAULT_EAMID_VALUE,
            false);
        log_warning("Invalid value for key '%s' specified, fallback "
            "to default", tmp);
        free(tmp);
