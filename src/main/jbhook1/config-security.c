...
#define JBHOOK1_CONFIG_SECURITY_DEFAULT_MCODE_VALUE jbhook1_config_security_default_mcode
        ...
        struct jbhook1_config_security* config_security, struct cconfig* config)
{
    ...
    if (!cconfig_util_get_str(config, JBHOOK1_CONFIG_SECURITY_MCODE_KEY,
            mcode, sizeof(mcode) - 1, tmp_default)) {
        log_warning("Invalid value for key '%s' specified, fallback "
            "to default '%s'", JBHOOK1_CONFIG_SECURITY_MCODE_KEY, tmp_default);
    }

    mcode[8] = '\0';

    if (!security_mcode_parse(mcode, &config_security->mcode)) {
        log_warning("Invalid mcode '%s' specified, fallback to default '%s'", 
            mcode, tmp_default);
    
        memcpy(&config_security->mcode, 
