...
static HWND CDECL my_mwindow_create(HINSTANCE, void*, const char*, DWORD, 
    DWORD, BOOL);
static HWND (CDECL *real_mwindow_create)(HINSTANCE, void*, const char*, DWORD,
    DWORD, BOOL);
 ...
 */
static HWND CDECL my_mwindow_create(HINSTANCE hinstance, void* callback, 
        const char* window_title, DWORD window_width, DWORD window_height,
        BOOL fullscreen)
{
    ...
    if (!cconfig_hook_config_init(config, JBHOOK1_INFO_HEADER "\n" JBHOOK1_CMD_USAGE, CCONFIG_CMD_USAGE_OUT_DBG)) {
    ...
    jb_io_set_loggers(log_impl_misc, log_impl_info, log_impl_warning,
            log_impl_fatal);
    ...
    jbhook1_p3io_init(&config_security.mcode, &config_eamuse.pcbid,
        &config_eamuse.eamid);
    ...
    return real_mwindow_create(hinstance, callback, window_title, window_width,
