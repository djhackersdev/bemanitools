...
static void _launcher_env_game_dir_verify()
{
    ...
    if (!path_exists(modules_dir)) {
        log_fatal("Cannot find 'modules' directory in current working directory: %s", cwd);
    }

    if (!path_exists(prop_dir)) {
        log_fatal("Cannot find 'prop' directory in current working directory: %s", cwd);
    }
}

static void _launcher_config_load(const char *config_path, struct launcher_config *config)
{
    ...
    log_info("Loading launcher configuration from file: %s", config_path);

    launcher_config_init(config);
    
    property = property_util_load(config_path);
    launcher_config_load(property, config);

    if (config->debug.log_property_configs) {
        log_misc("launcher-config");
        property_util_log(property);
    }

    property_util_free(property);
}

static void _launcher_bootstrap_config_options_override(struct launcher_bootstrap_config *config, const struct options_bootstrap *options)
{
    log_assert(config);
    log_assert(options);

    if (options->config_path) {
        log_misc("Command line override bootstrap configuration from file: %s", options->config_path);
    ...
    if (options->selector) {
        log_misc("Command line override bootstrap selector: %s", options->selector);
...
static void _launcher_ea3_ident_config_options_override(
        ...
        const struct options_eamuse *options)
{
    log_assert(config);
    log_assert(options);
...
static void _launcher_hook_config_options_override(
        ...
        const struct options_hook *options)
{
    size_t i;
    const char *dll;
    ...
    for (i = 0; i < options->hook_dlls.nitems; i++) {
        ...
        if (!launcher_config_add_hook_dll(config, dll)) {
            log_warning("Adding override hook dll '%s' failed (max supported limit exceeded), ignored", dll);
        }
    }
    ...
    for (i = 0; i < options->before_hook_dlls.nitems; i++) {
        ...
        if (!launcher_config_add_before_hook_dll(config, dll)) {
            log_warning("Adding override before hook dll '%s' failed (max supported limit exceeded), ignored", dll);
        }
    }
    ...
    for (i = 0; i < options->iat_hook_dlls.nitems; i++) {
        ...
        if (!launcher_config_add_iat_hook_dll(config, dll)) {
            log_warning("Adding override iat hook dll '%s' failed (max supported limit exceeded), ignored", dll);
        }
    }
}

static void _launcher_debug_config_options_override(struct launcher_debug_config *config, const struct options_debug *options)
{
    log_assert(config);
    log_assert(options);
        ...
        const struct options *options)
{
    ...
    // Apply command line overrides on all launcher owned configuration parameters
    _launcher_bootstrap_config_options_override(&config->bootstrap, &options->bootstrap);
...
static void _launcher_config_full_resolved_log(const struct launcher_config *config)
{
    if (config->debug.log_property_configs) {
...
static void _launcher_remote_debugger_trap(
...
static void _launcher_bootstrap_log_config_options_override(
        struct bootstrap_log_config *config,
        const struct options_log *options)
{
    ...
    if (options->level) {
        log_misc("Command line override bootstrap log level: %d", *(options->level));
    ...
    if (options->file_path) {
        log_misc("Command line override bootstrap log file: %s", options->file_path);
    ...
    const struct bootstrap_config *bootstrap_config)
{
    ...
    if (!str_eq(bootstrap_config->startup.log.level, "misc")) {
        if (launcher_config->debug.log_property_configs) {
            log_warning("Logging of property configs enabled, but requires misc log level, current log level: %s", bootstrap_config->startup.log.level);
        }
    }
}

static void _launcher_before_hook_dlls_load(const struct launcher_hook_config *config)
{
    int i;
        ...
        bool log_property_configs)
{
    ...
    if (!ea3_ident_config_hardid_is_defined(config)) {
        log_misc("No no hardid defined in ea3-ident-config, derive from ethernet");
    ...
    const struct launcher_hook_config *hook_config)
{
    ...
    for (i = 0; i < LAUNCHER_CONFIG_MAX_HOOK_DLL; i++) {
        if (strlen(hook_config->before_hook_dlls[i]) > 0) {
            *array_append(const char *, &iat_hook_dlls) = (const char *) &hook_config->before_hook_dlls[i];
        }
    }
    ...
    struct ea3_ident_config *ea3_ident_config)
{
    ...
    if (proc_is_running_as_admin_user()) {
        log_warning("Not running as admin user. Launcher and games require elevated privileges to run correctly");
    }

    if (options->launcher.config_path) {
        _launcher_config_load(
    ...
    _launcher_bootstrap_config_load(&launcher_config->bootstrap, bootstrap_config);
    _launcher_bootstrap_log_config_options_override(&bootstrap_config->startup.log, &options->log);
    ...
    _launcher_ea3_ident_config_options_override(ea3_ident_config, &options->eamuse);
    ...
    struct ea3_ident_config *ea3_ident_config)
{
    ...
    _launcher_bootstrap_module_init(&bootstrap_config->startup.module, &launcher_config->hook);
    ...
    bootstrap_module_game_init(
        &bootstrap_config->startup.module,
        ea3_ident_config);
