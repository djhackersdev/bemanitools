...
#define NODE_LOADING_FATAL(subnode) log_fatal("%s/%s: Node loading", ROOT_NODE, subnode);

static struct property* _launcher_config_layered_config_nodes_load(
    struct property_node *node)
{
    ...
    while (cur) {
        if (cnt >= MAX_LAYER_CONFIG_NODES) {
            log_fatal("Exceeding max supported config nodes for layering, max is %d", MAX_LAYER_CONFIG_NODES);
        }

        res = property_node_refer(NULL, cur, "kind@", PROPERTY_TYPE_ATTR, kind, sizeof(kind));
        ...
        if (!strcmp(kind, "file")) {
            ...
            // The nested child is the actual root of the inline, not the outer <config> node
            cur = property_node_traversal(cur, TRAVERSE_FIRST_CHILD);

            config_property[cnt] = property_util_clone(cur);
        } else {
            log_fatal("Unsupported 'kind' attribute value '%s' of config node", kind);
        }
    ...
    merged_property = property_util_merge((struct property **) &config_property[0], cnt);
        ...
        struct launcher_bootstrap_config *config)
{
    ...
    res = property_node_refer(NULL, node, "selector", PROPERTY_TYPE_STR, config->selector, sizeof(config->selector));
...
static void _launcher_config_hook_load(
        struct property_node *node,
        struct launcher_hook_config *config)
{
    log_assert(node);
    log_assert(config);

    _launcher_config_hook_dlls_parse(node, "hook_dlls/dll", config->hook_dlls);
    _launcher_config_hook_dlls_parse(node, "before_hook_dlls/dll", config->before_hook_dlls);
    _launcher_config_hook_dlls_parse(node, "iat_hook_dlls/dll", config->iat_hook_dlls);
}


static void _launcher_config_debug_load(
        struct property_node *node,
        struct launcher_debug_config *config)
{
    log_assert(node);
    log_assert(config);
    ...
    struct launcher_config *config)
{
    ...
    error = property_node_refer(
            ...
            "version@",
            PROPERTY_TYPE_ATTR,
            &config->version, sizeof(uint32_t));
    ...
    // if (config->version != 1) {
    //     log_fatal("Unsupported version of launcher configuration: %d", config->version);
    // }
    ...
    node = property_search(NULL, root_node, "debug");

    if (node) {
        _launcher_config_debug_load(node, &config->debug);
    }
}

bool launcher_config_add_hook_dll(struct launcher_config *config, const char *path)
{
    int i;
...
bool launcher_config_add_before_hook_dll(struct launcher_config *config, const char *path)
{
    int i;
    ...
    str_cpy(config->hook.before_hook_dlls[i], sizeof(config->hook.before_hook_dlls[i]), path);
    
    return true;
}

bool launcher_config_add_iat_hook_dll(struct launcher_config *config, const char *path)
{
    int i;
    ...
    str_cpy(config->hook.iat_hook_dlls[i], sizeof(config->hook.iat_hook_dlls[i]), path);
