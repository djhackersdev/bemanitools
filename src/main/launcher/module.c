...
static bool module_replace_dll_iat(HMODULE hModule, const struct array *iat_hook_dlls)
{
    log_assert(hModule);
    ...
    struct property_node *app_params_node)
{
    ...
    struct property *prop = property_util_cstring_load("<param><io>p3io</io></param>");
    ...
    memcpy(ea3_ident_config->model, sidcode_short + 0, sizeof(ea3_ident_config->model) - 1);
    ea3_ident_config->dest[0] = sidcode_short[3];
    ea3_ident_config->spec[0] = sidcode_short[4];
    ea3_ident_config->rev[0] = sidcode_short[5];
    memcpy(ea3_ident_config->ext, sidcode_short + 6, sizeof(ea3_ident_config->ext));
