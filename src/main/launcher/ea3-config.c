...
void ea3_ident_hardid_from_ethernet(struct ea3_ident *ident)
{
    ...
    if (result < 0) {
        log_fatal("avs_net_ctrl call to get MAC address returned error: %d",
                result);
    }
    ...
    hex_encode_uc(netif.mac_addr, sizeof(netif.mac_addr),
            ident->hardid + 4, sizeof(ident->hardid) - 4);
}
        ...
        const struct module_context *module, struct property_node *app_config)
{
    ...
    str_format(security_code, lengthof(security_code), "G*%s%s%s%s",
            ident->model, ident->dest, ident->spec, ident->rev);
    ...
    str_format(sidcode_short, lengthof(sidcode_short), "%s%s%s%s%s",
            ident->model, ident->dest, ident->spec, ident->rev, ident->ext);
    ...
    str_format(sidcode_long, lengthof(sidcode_long), "%s:%s:%s:%s:%s",
            ident->model, ident->dest, ident->spec, ident->rev, ident->ext);
