...
int main(int argc, char **argv)
{
    ...
    p3io_usb_read_version(handle, version);

    printf("version: %s\n", version);
    
    while (true) {
        bool ret = p3io_usb_read_jamma(handle, &jamma);

        if (!ret) {
            printf("Reading jamma failed\n");
            break;
        }

        printf(">>> %X %X %X\n", jamma[0], jamma[1], jamma[2]);
