...
static const char ezusb_iidx_emu_node_serial_eamio_mapping[EAM_IO_KEYPAD_COUNT] = {
    '0',
    '1',
    '4',
    '7',
...
static struct ezusb_iidx_emu_node_serial_emulation_state ezusb_iidx_emu_node_serial_emulation_state[2];

static void ezusb_iidx_emu_node_serial_dump_buf_log(const char* header_msg,
        const uint8_t* buffer, uint16_t length);

static void ezusb_iidx_emu_node_serial_exec_req_resp(const uint8_t* buffer_in,
        uint16_t buffer_in_length, uint8_t* buffer_out, uint16_t* buffer_out_length);
...
void ezusb_iidx_emu_node_serial_init(void)
{
    ...
    memset(ezusb_iidx_emu_node_serial_read_buf, 0xFF, sizeof(ezusb_iidx_emu_node_serial_read_buf));
    ...
    memset(ezusb_iidx_emu_node_serial_emulation_state, 0,
        sizeof(struct ezusb_iidx_emu_node_serial_emulation_state) * 2);

    for (uint8_t i = 0; i < 2; i++) {
        InitializeCriticalSection(&ezusb_iidx_emu_node_serial_emulation_state[i].card_cs);
    }

    ezusb_iidx_emu_node_serial_emu_thread =
        thread_create(ezusb_iidx_emu_node_serial_emu_thread_proc, NULL, 0x4000, 0);
}
...
bool ezusb_iidx_emu_node_serial_read_packet(struct ezusb_iidx_msg_bulk_packet* pkg)
{
    ...
    /* calc data size */
    buffer_offset = ezusb_iidx_emu_node_serial_read_buf_page * sizeof(pkg->payload);
    ...
    memcpy(pkg->payload, ezusb_iidx_emu_node_serial_read_buf + buffer_offset, data_length);
...
bool ezusb_iidx_emu_node_serial_write_packet(const struct ezusb_iidx_msg_bulk_packet* pkg)
{
    ...
    buffer_offset = ezusb_iidx_emu_node_serial_wrote_buf_page * sizeof(pkg->payload);
    ...
    memcpy(ezusb_iidx_emu_node_serial_write_buf + buffer_offset, pkg->payload, data_length);

    /* serial buffer is filled, execute request/response chain
       dunno if this is a good spot to do this... */

    if (execute) {
        ezusb_iidx_emu_node_serial_exec_req_resp(ezusb_iidx_emu_node_serial_write_buf,
            ezusb_iidx_emu_node_serial_write_buf_data_len, ezusb_iidx_emu_node_serial_read_buf,
...
void ezusb_iidx_emu_node_serial_set_card_attributes(uint8_t card_type, bool used_card,
        const char* card_version)
{
    ...
    memcpy(ezusb_iidx_emu_node_serial_card_attr_version, card_version,
...
static uint8_t calc_serial_buffer_checksum(const uint8_t* buffer, uint16_t length)
{
    uint8_t checksum = 0;
...
{
    *msg_out_len = 4 + 1;
    struct ezusb_iidx_emu_node_serial_msg* resp = malloc(*msg_out_len);
...
static struct ezusb_iidx_emu_node_serial_msg* process_serial_msg(
        ...
        uint16_t msg_in_len, uint16_t* msg_out_len)
{
    //log_misc("!!! Serial processing msg_cmd %02X, node_cmd %02X, node_id %02X",
    ...
    switch (msg_in->msg_cmd) {
            ...
            switch (msg_in->node_cmd) {
                case H8_CMD_NODE_ENUM:
                {
                    ...
                    *msg_out_len = 4 + 1;
                    ...
                    /* This response is not from a specific node, id needs to be 0 */
                    resp->node_id = 0;
                    resp->node_cmd = msg_in->node_cmd;
                    resp->payload_len = 1;
                ...
                case H8_CMD_GET_VERSION:
                {
                    ...
                    *msg_out_len = 4 + 13;
                    ...
                    resp->node_id = msg_in->node_id;
                    resp->node_cmd = msg_in->node_cmd;
                    /* Payload length is not 13 here, needs to 5 (comment length?)
                       Might be a bug in the ezusb code */
                    resp->payload_len = 5;
                ...
                case H8_CMD_PROG_EXEC:
                {
                    ...
                    *msg_out_len = 4 + 1;
                    ...
                    uint8_t state =
                        ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1]
                        .state_current;

                    if (state == EMU_STATE_UNINIT) {
                        InterlockedExchange(
                            &ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1]
                                .state_current, EMU_STATE_INIT);
                ...
                default:
                    log_warning("Serial invalid node cmd on h8 req: %02X",
                            msg_in->node_cmd);
                    ezusb_iidx_emu_node_serial_dump_buf_log("", (const uint8_t*) msg_in,
                        msg_in_len);
                    return NULL;
            }
        }
            ...
            switch (msg_in->node_cmd) {
                ...
                case NODE_CMD_CARD_RW_UNIT_GET_STATUS:
                {
                    ...
                    *msg_out_len = 4 + 1;
                    ...
                    if (    ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1]
                        ...
                        if (    ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1]
                            ...
                            /* needs to be set if either front or back is triggered */
                ...
                {
                    switch (msg_in->card_slot_state_req.ezusb_iidx_emu_node_serial_card_slot_state) {
                        case CARD_SLOT_STATE_CLOSE:
                            log_misc("CARD_SLOT_STATE_CLOSE, node %d",
                                    msg_in->node_id);
                        ...
                        case CARD_SLOT_STATE_OPEN:
                            log_misc("CARD_SLOT_STATE_OPEN, node %d",
                                    msg_in->node_id);
                        ...
                        case CARD_SLOT_STATE_EJECT:
                            log_misc("CARD_SLOT_STATE_EJECT, node %d",
                                    msg_in->node_id);
                        ...
                        case CARD_SLOT_STATE_FORMAT:
                            log_misc("CARD_SLOT_STATE_FORMAT, node %d",
                                    msg_in->node_id);
                            break;

                        case CARD_SLOT_STATE_READ:
                            log_misc("CARD_SLOT_STATE_READ, node %d",
                                    msg_in->node_id);
                        ...
                        case CARD_SLOT_STATE_WRITE:
                            log_misc("CARD_SLOT_STATE_WRITE, node %d",
                                    msg_in->node_id);
                            break;

                        default:
                            log_warning("Invalid card slot state %d for node %d",
                                msg_in->card_slot_state_req
                                .ezusb_iidx_emu_node_serial_card_slot_state, msg_in->node_id);
                    }
                ...
                case NODE_CMD_CARD_READ:
                {
                    *msg_out_len = 4 + 129;
                    ...
                    if (ezusb_iidx_emu_node_serial_write_loopback_card_buf[msg_in->node_id - 1] == NULL) {
                        ...
                        while (state == EMU_STATE_REQ_CARD) {
                            state = ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1]
                                .state_current;
                            Sleep(5);
                        }
                        ...
                        if (state == EMU_STATE_CARD_READ) {
                            ...
                            log_info("Read card %02X%02X%02X%02X%02X%02X%02X%02X",
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[0],
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[1],
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[2],
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[3],
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[4],
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[5],
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[6],
                                ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1].card_id[7]);
                            ...
                            ezusb_iidx_emu_card_mag_generate_data(card,
                                card_invert,
                            ...
                            /* ezusb_iidx_emu_node_serial_dump_buf_log("card data",
                                resp->card_read_resp.data,
                                sizeof(struct ezusb_iidx_emu_card_mag_data)); */

                            InterlockedExchange(
                                &ezusb_iidx_emu_node_serial_emulation_state[msg_in->node_id - 1]
                                .state_current, EMU_STATE_LOOP);
                        ...
                        memcpy(resp->card_read_resp.data,
                                ezusb_iidx_emu_node_serial_write_loopback_card_buf[msg_in->node_id - 1],
                                MAG_CARD_DATA_SIZE);

                        free(ezusb_iidx_emu_node_serial_write_loopback_card_buf[msg_in->node_id - 1]);
                        ezusb_iidx_emu_node_serial_write_loopback_card_buf[msg_in->node_id - 1] = NULL;
                    }
                ...
                case NODE_CMD_CARD_WRITE:
                {
                    /* i hate this solution, but it gets rid of the problem that
                       ...
                       the write and loop it back on the next read (next read only) */
                    ...
                    ezusb_iidx_emu_node_serial_write_loopback_card_buf[msg_in->node_id - 1] =
                        malloc(MAG_CARD_DATA_SIZE);
                    memcpy(ezusb_iidx_emu_node_serial_write_loopback_card_buf[msg_in->node_id - 1],
                            msg_in->card_write_req.data, MAG_CARD_DATA_SIZE);
                ...
                case NODE_CMD_KEYBOARD_GET_BUFFER_SIZE:
                {
                    *msg_out_len = 4 + 2;
                    ...
                    if (ezusb_iidx_emu_node_serial_emulation_state[node].keypad_buf != 0) {
                        bufferSize = 1;
                    }
                    ...
                    if (bufferSize < 3) {
                        ...
                        resp->keyboard_get_buffer_size_resp.buffer_size_type = 3;
                    } else if (bufferSize <= 8) {
                        resp->keyboard_get_buffer_size_resp.buffer_size_type = 4;
                    } else if (bufferSize <= 16) {
                        resp->keyboard_get_buffer_size_resp.buffer_size_type = 5;
                    } else if (bufferSize <= 32) {
                        resp->keyboard_get_buffer_size_resp.buffer_size_type = 6;
                    } else {
                        /* avoid overflow, clip everything else */
                        resp->keyboard_get_buffer_size_resp.buffer_size_type = 7;
                    }
                ...
                case NODE_CMD_KEYBOARD_READ_DATA:
                {
                    *msg_out_len = 4 + 2;
                    ...
                    resp->payload_len = msg_in->keyboard_read_data_req.buffer_size_type;
                    ...
                    {
                        case 0:
                            maxBufferSize = 0; break;
                        case 1:
                            maxBufferSize = 1; break;
                        case 2:
                            maxBufferSize = 2; break;
                        case 3:
                            maxBufferSize = 4; break;
                        case 4:
                            maxBufferSize = 8; break;
                        case 5:
                            maxBufferSize = 16; break;
                        case 6:
                            maxBufferSize = 32; break;
                        case 7:
                            maxBufferSize = 64; break;
                        default:
                            log_fatal("Invalid buffer size %d for reading keyboard data.",
                    ...
                    if (maxBufferSize > 0) {
                        uint8_t node = msg_in->node_id - 1;
                        resp->keyboard_read_data_resp.data[0] =
                            InterlockedExchange(
                ...
                default:
                    log_warning("Serial invalid node cmd on node req: %02X",
                            msg_in->node_cmd);
                    ezusb_iidx_emu_node_serial_dump_buf_log("", (const uint8_t*) msg_in,
                        msg_in_len);
                    return NULL;
        ...
        default:
            ...
            ezusb_iidx_emu_node_serial_dump_buf_log("", (const uint8_t*) msg_in, msg_in_len);
            return NULL;
    }
}
        ...
        uint16_t buffer_in_length, uint8_t* buffer_out, uint16_t* buffer_out_length)
{
    ...
    /* Before we do any processing:
       Filter garbage/broken messages. Maybe caused by buggy code (?) some messages
       during the uart init phase start with 0x00 and don't have a valid checksum.
       However, the game wants a response for that but doesn't care about the contents
       so we just throw the trash back where it came from */
    ...
    */
    ...
    if (buffer_in_length > 0 && buffer_in[0] != HEADER_BYTE) {
        ...
        ezusb_iidx_emu_node_serial_dump_buf_log("", (const uint8_t*) buffer_in, buffer_in_length);
        *buffer_out_length = 0;
        return;
    }
    ...
    if (buffer_in[buffer_in_length - 1] != calc_serial_buffer_checksum(
                buffer_in + 1, buffer_in_length - 2)) {
        log_warning("Serial message invalid checksum");
        ezusb_iidx_emu_node_serial_dump_buf_log("", buffer_in, buffer_in_length);
        *buffer_out_length = 0;
        return;
    }
    ...
    if (msg_buf_in_len == 2 && msg_buf_in[0] == HEADER_BYTE &&
        ...
        buffer_out[msg_buf_out_len + 1] = calc_serial_buffer_checksum(msg_buf_out,
                msg_buf_out_len);
        *buffer_out_length = msg_buf_out_len + 2;
        ...
        if (msg_in->msg_cmd == CMD_H8_REQ && msg_in->node_cmd == H8_CMD_NODE_ENUM) {
            uint8_t* msg_buf_out = &buffer_out[1];
            uint16_t msg_buf_out_len = 0;
            struct ezusb_iidx_emu_node_serial_msg* msg_out =
                process_serial_msg((const struct ezusb_iidx_emu_node_serial_msg*) msg_buf_in,
                        msg_buf_in_len, &msg_buf_out_len);
            ...
            buffer_out[0] = HEADER_BYTE;
            buffer_out[msg_buf_out_len + 1] = calc_serial_buffer_checksum(
                    msg_buf_out, msg_buf_out_len);
            *buffer_out_length = msg_buf_out_len + 2;
        } else {
...
{
    InterlockedCompareExchange(&ezusb_iidx_emu_node_serial_emulation_state[node].keypad_buf,
        convert_keyboard_char(key), 0);
}
...
static int ezusb_iidx_emu_node_serial_emu_thread_proc(void* ctx)
{
    ...
    while (init < 2) {
        for (uint8_t i = 0; i < 2; i++) {
            if (ezusb_iidx_emu_node_serial_emulation_state[i].state_current == EMU_STATE_INIT) {
                InterlockedExchange(
                        &ezusb_iidx_emu_node_serial_emulation_state[i].state_current,
                        EMU_STATE_LOOP);
                init++;
            }
        }
    ...
    while (true) {
        for (uint8_t node = 0; node < 2; node++) {
            ...
            for (uint8_t i = 0; i < sizeof(ezusb_iidx_emu_node_serial_eamio_mapping); ++i) {
                if (keypad_rise & (1 << i)) {
                    keyboard_add_key_press_to_buffer(node,
                        ezusb_iidx_emu_node_serial_eamio_mapping[i]);
                }
            }
            ...
            switch (ezusb_iidx_emu_node_serial_emulation_state[node]
                ...
                case EMU_CARD_SLOT_STATE_EJECT:
                    ...
                    EnterCriticalSection(
                        &ezusb_iidx_emu_node_serial_emulation_state[node].card_cs);

                    /* invalidate card id */
                    memset(ezusb_iidx_emu_node_serial_emulation_state[node].card_id, 0,
                        CARD_ID_LEN);
                ...
                case EMU_CARD_SLOT_STATE_READ:
                    ...
                    if (    ezusb_iidx_emu_node_serial_emulation_state[node]
                        ...
                        if (    !eam_io_read_card(node,
                                ezusb_iidx_emu_node_serial_emulation_state[node].card_id,
                                CARD_ID_LEN)) {
                            memset(ezusb_iidx_emu_node_serial_emulation_state[node].card_id,
                                0, CARD_ID_LEN);
                            InterlockedExchange(
                                &ezusb_iidx_emu_node_serial_emulation_state[node].state_current,
                                EMU_STATE_ERROR);
                        } else {
                            InterlockedExchange(
                                &ezusb_iidx_emu_node_serial_emulation_state[node].state_current,
