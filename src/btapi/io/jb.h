...
/* Initialize your JB IO emulation DLL. Thread management functions are
   ...
   bt_io_jb_set_loggers(). You will also need to pass these thread management
...
/* Get panel button state. Will return either any button being pressed, or a
   particular panel corner depending on previous call to bt_io_jb_set_panel_mode */
...
/* Open or close the coin chute - true will redirect all coins to the return
   slot. Required for bt_io_jb_set_panel_mode to operate correctly on p4io.
