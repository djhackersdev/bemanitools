...
typedef int (*btsdk_core_thread_create_impl_t)(
    int (*proc)(void *), void *ctx, uint32_t stack_sz, unsigned int priority);
typedef void (*core_thread_join_impl_t)(int thread_id, int *result);
typedef void (*core_thread_destroy_impl_t)(int thread_id);

struct core_thread_impl {
    core_thread_create_impl_t thread_create;
    core_thread_join_impl_t thread_join;
    core_thread_destroy_impl_t thread_destroy;
};
